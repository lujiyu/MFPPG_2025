...
class FeatGen(Dataset):
    ...
    def __init__(self, mode=cfg.mode, train_joblib=cfg.train_joblib, test_joblib=cfg.test_joblib,
    ...
    def compute_logmel(self, y):
        mel_spec = librosa.feature.melspectrogram(y=y, sr=cfg.fs, n_mels=cfg.n_mels, n_fft=cfg.n_fft,
    ...
    def extract_all_features(self, audio_path):
        ...
        mel = librosa.feature.melspectrogram(y=y, sr=cfg.fs, n_mels=cfg.n_mels, n_fft=cfg.n_fft,
                                             hop_length=cfg.hop_length)
    ...
    def feat_process(self):
        ...
        for idx, folder in enumerate(folder_list):
            ...
            inner_progress_bar = tqdm(enumerate(wav_list), total=len(wav_list),
                                      desc=f"Feat Gen: ({idx + 1}/{len(folder_list)}-{folder})", position=0, leave=True)
            ...
            folder_data = [((logmel, tsr, fsr, tkeo), label, detect) for logmel, tsr, fsr, tkeo, label, detect in
                           zip(norm_logmel_specs, norm_tsr_specs, norm_fsr_specs, norm_tkeo_specs, label_list,
                               detect_list)]
...
if __name__ == '__main__':
    ...
    for mode_ in modes:
        for (train_joblib, test_joblib) in zip(train_joblibs, test_joblibs):
            ...
            train_data = FeatGen(mode=mode_, train_joblib=train_joblib, test_joblib=test_joblib, cur_domain='source',
                                 year='2024')
